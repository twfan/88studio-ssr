            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="container w-3/4 p-5 mx-auto flex flex-col">
                    ...
                    <a href="{{ route('admin.banners.create') }}">
                      <button type="button" class="px-3 py-2 text-sm bg-slate-400 rounded text-white flex flex-row items-center content-center gap-1"><i class="w-4 h-4" data-feather="plus"></i> Create a new banner</button>
                    </a>
                    </div>
                    <div class="w-full h-full bg-white p-3 rounded-lg shadow">
                        <table class="table-fixed w-full border-collapse rounded-md">
                            <thead>
                              <tr>
                                <th class="rounded-s-md bg-slate-100 text-slate-500 text-left p-3">Name</th>
                              ...
                              <tr>
                                ...
                                <td class="px-3 py-5 border-b-8 border-white ">
                                    <div class="flex flex-row gap-2 h-full">
                                        <a href="{{ route('admin.banners.edit', $item->id) }}"  class="btn btn-danger px-3 py-2 border rounded flex content-center items-center justify-center gap-1 bg-yellow-400 text-white" >
                                          <i class="w-4 h-4" data-feather="edit-2"></i> Edit
                                        </a>
                                        <form name="deleteForm{{$item->id}}" action="{{ route('admin.banners.destroy', $item->id) }}" method="POST" style="display: inline;">
                                          @csrf
                                          @method('DELETE')
